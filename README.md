...
## 🏗️ 技術選定と設計思想 (Architecture Decision Records)

本プロジェクトでは、開発効率と保守性を最大化するために以下の技術を選定しました。

### パッケージ管理： uv

- 採用理由：高速な依存関係解決と再現性の担保
- 詳細：従来の'pip' 'poetry'と比較して圧倒的に高速なセットアップが可能です。'uv.lock'による厳密なバージョン固定により、環境差異による動かないトラブルを未然に防ぎます

### ブラウザ操作： Selenium

- 採用理由：動的コンテンツへの確実なアクセス
- 詳細：操作対象のサイト（Lacicra）はJavaScriptを用いて動的にDOMを生成しています。静的なHTTPリクエスト（'requests'）では対応できないログイン処理や動的なフォーム操作を確実に行うため、ブラウザオートメーションを採用しました

### 静的解析： Ruff

- 採用理由：コード品質の維持と開発スピードの両立
- 詳細：Pythonのリンター・フォーマッターとして、Rust製の高速なツールであるRuffを採用しました。厳格な型チェックとスタイル統一を自動化し、バグの早期発見と可読性の向上を実現しています。

リンター：文法エラー、未使用変数、バグの可能性、型違反などを警告
フォーマッター：インデント、スペース、改行、クォートの種類などを統一ルールに書き換える
...
### 1. Chromeのバージョンエラー
Selenium実行時に「SessionNotCreatedException」が発生する場合、Chromeブラウザのバージョンが更新されている可能性があります。
- 対策: `uv run pip install --upgrade webdriver-manager` を実行するか、Chromeを最新版にアップデートしてください。
