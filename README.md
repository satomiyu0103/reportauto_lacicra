...
- **詳細**: **従来の'pip' 'poetory'と比較して圧倒的に高速なセットアップが可能です。'uv.lock'による厳密なバージョン湖底により、環境際による動かないトラブルを未然に防ぎます**

### ブラウザ操作： Selenium

- **採用理由**: **動的コンテンツへの確実なアクセス**
- **詳細**: **捜査対象のサイト（Lacicra）はJavaScriptを用いて動的にDOMを生成しています。静的なHTTPリクエスト（'requests'）では対応できないログイン処理や動的なフォーム操作を確実に行うため、ブラウザオートメーションを採用しました**
...
フォーマッター：インデント、スペース、開業、くぉーとの種類などを統一ルールに書き換える
